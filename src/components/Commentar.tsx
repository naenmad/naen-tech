import React, { useState, useEffect, useRef, useCallback, memo, FormEvent, ChangeEvent } from "react";
import {
  addDoc,
  collection,
  onSnapshot,
...
// Tipe untuk properti komponen Comment
interface CommentProps {
  comment: CommentType;
  formatDate: (timestamp: { toDate: () => Date } | null) => string;
  onLike: (id: string) => void;
}

// Komponen untuk menampilkan komentar
const Comment = memo(({ comment, formatDate, onLike }: CommentProps) => (
...
// Tipe untuk properti CommentForm
...
const Commentar = () => {
  ...
  const formatDate = useCallback((timestamp: { toDate: () => Date } | null) => {
    if (!timestamp) return "";
        ...
        <div className="space-y-4 h-[300px] overflow-y-auto custom-scrollbar" data-aos="fade-up" data-aos-delay="200">
          {comments.length === 0 ? (
            ...
            comments.map((comment) => (
              <Comment key={comment.id} comment={comment} formatDate={formatDate} onLike={handleLike} />
            ))
          )}
        </div>
      </div>
